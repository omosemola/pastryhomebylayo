...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Spiral Banner Animation on Scroll
    const spiralContainer = document.querySelector('.spiral-banner-container');
    const spiralImg = document.querySelector('.spiral-banner-img');

    if (spiralContainer && spiralImg) {
        window.addEventListener('scroll', () => {
            const rect = spiralContainer.getBoundingClientRect();
            const windowHeight = window.innerHeight;

            // Calculate visibility (0 to 1)
            // 0: below viewport, 1: fully in viewport or above
            // We want the effect to happen as it centers

            if (rect.top < windowHeight && rect.bottom > 0) {
                // Determine progress of scroll through the element
                // Center of element relative to center of viewport
                const centerOffset = (windowHeight / 2) - (rect.top + rect.height / 2);
                const progress = centerOffset / windowHeight; // Range roughly -0.5 to 0.5

                // Spiral Effect: 
                // Scale: 1.5 -> 1.0 (zoom out)
                // Rotate: 10deg -> 0deg

                // Normalize progress to be 0 at entry, 1 at fully scrolled past?
                // Let's make it responsive to being "centered"

                const scale = Math.max(1, 1.5 - (rect.top < windowHeight ? (windowHeight - rect.top) * 0.001 : 0));
                // Simple scroll bound:
                // When rect.top is high (entering), scale is high (zoomed in)
                // When rect.top is low (scrolled up), scale decreases (zooms out)

                const scrollY = window.scrollY;
                const offset = scrollY * 0.05; // speed

                // Let's use a simpler formula for spiral zoom out
                // Zoom out as we scroll down
                const baseScale = 1.0;
                const zoomFactor = 0.0005;
                const activeScale = 1.6 - (Math.max(0, window.scrollY - spiralContainer.offsetTop + windowHeight) * zoomFactor);
                const finalScale = Math.max(1, activeScale);

                const rotate = Math.max(0, 10 - (Math.max(0, window.scrollY - spiralContainer.offsetTop + windowHeight) * 0.02));

                spiralImg.style.transform = `scale(${finalScale}) rotate(${rotate}deg)`;
            }
        });
    }

    // 2. Events Slider Logic
    const eventsTrack = document.getElementById('events-track');
    if (eventsTrack) {
        // Populate Images
        const eventImages = [];
        for (let i = 1; i <= 10; i++) {
            eventImages.push(`./assets/events/events${i}.jpg`);
        }

        eventImages.forEach((src, index) => {
            const slide = document.createElement('div');
            slide.className = 'event-slide';
            slide.innerHTML = `<img src="${src}" alt="Event ${index + 1}">`;
        ...
        function updateEventSlider() {
            ...
            const gap = 16; // 1rem
            const containerCenter = eventsTrack.parentElement.offsetWidth / 2;
            const slideCenter = slideWidth / 2;

            // Calculate transform to center the active slide
            const offset = (eventIndex * (slideWidth + gap)) - containerCenter + slideCenter;
        ...
        function startAutoSwipe() {
            autoSwipeInterval = setInterval(() => {
                eventIndex = (eventIndex + 1) % totalSlides;
                updateEventSlider();
            }, 2000); // 2s interval
        }
        ...
        // Init
        // Wait specifically for images to load or just init layout
        setTimeout(() => {
            updateEventSlider();
            startAutoSwipe();
        }, 500);
        ...
        // Touch / Hover Pause
        eventsTrack.addEventListener('mouseenter', stopAutoSwipe);
        eventsTrack.addEventListener('mouseleave', startAutoSwipe);

        eventsTrack.addEventListener('touchstart', stopAutoSwipe);
        eventsTrack.addEventListener('touchend', startAutoSwipe);
    }
});
